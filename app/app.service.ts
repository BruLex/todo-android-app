...
export class AppService {
    ...
    private cacheImageFolder: string = path.join(ad.getApplicationContext().getExternalFilesDir(null).getAbsolutePath(), "images");
    ...
    constructor() {
        (new Sqlite("todos.db")).then((db): void => {
            // db.execSQL("DROP TABLE todos");
            db.execSQL("CREATE TABLE IF NOT EXISTS todos (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, body TEXT)").then(id => {
                this.database = db;
                // this.fetch();
    ...
    fetch(): void {
        this.database.all("SELECT * FROM todos").then((rows): void => {
            ...
            this.fetchFilesById(this.todos.length - 1);
        }, (error): void => {
    ...
    fetchFilesById(id: number): void {
        this.todos[id].imagesPaths = Folder.fromPath(path.join(this.rootImageFolder, id.toString()))
            .getEntitiesSync()
            .map(img => img.path);
    }
    ...
    getAllCachedFiles(): string[] {
        return Folder.fromPath(this.cacheImageFolder).getEntitiesSync().map(o => o.path);
    }
    ...
    copyCacheToDirectory(id: number | string): void {
        this.getAllCachedFiles().forEach(cachePath => {
            ...
            File.fromPath(filePath).writeSync(File.fromPath(cachePath).readSync()).then(() => console.log("Image saved from cache successfully!"));
        });
    }

    addImages(id: number | string, files: ImageAsset[]): void {
        files.forEach(file =>
    ...
    uniqueID(): string {
        const random4UUID: () => string = (): string =>
            Math.random()
                ...
                .join('')
                .slice(-4);
        return random4UUID() + random4UUID() +
            '-' + random4UUID() +
            '-' + random4UUID() +
            '-' + random4UUID() +
            '-' + random4UUID() + random4UUID() + random4UUID();
    }
}
