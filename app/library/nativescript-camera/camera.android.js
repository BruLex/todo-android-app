...
exports.takePicture = function(options) {
    return new Promise(function(resolve, reject) {
        try {
            ...
            if (options) {
                saveToGallery_1 = types.isNullOrUndefined(options.saveToGallery)
                    ? saveToGallery_1
                ...
                shouldKeepAspectRatio_1 = types.isNullOrUndefined(options.keepAspectRatio)
                    ? shouldKeepAspectRatio_1
            ...
            if (saveToGallery_1) {
                ...
                const folderPath = fileSystem.path.join(utils.ad
                    .getApplicationContext()
                    .getExternalFilesDir(null)
                    .getAbsolutePath(), 'images');
                fileSystem.Folder.fromPath(folderPath);
                picturePath_1 = fileSystem.path.join(folderPath,dateStamp + '.jpg');
                nativeFile = new java.io.File(picturePath_1);
            }
            ...
            if (sdkVersionInt >= 21) {
                tempPictureUri = FileProviderPackageName.FileProvider.getUriForFile(
                    ...
                    applicationModule.android.nativeApp.getPackageName() + '.provider',
                    nativeFile
                );
            } else {
                            ...
                            try {
                                var callback = new android.media.MediaScannerConnection.OnScanCompletedListener({
                                    onScanCompleted: function(path, uri) {
                                        if (trace.isEnabled()) {
                                            trace.write(
                                                'image from path ' + path + ' has been successfully scanned!',
